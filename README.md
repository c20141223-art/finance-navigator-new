...
- `stock_screener/emailer.py`：Gmail SMTP 寄件，憑證只走環境變數、無備援
  值、缺 secret 明確報錯。
- `stock_screener/web_export.py` + `web/index.html`：靜態儀表板。pipeline
  將四區塊輸出成 JSON（`web/data/latest.json`＋每日封存＋`index.json`
  日期索引），`web/index.html` 是純前端、無後端、無外部相依的行動優先
  頁面，用相對路徑讀 JSON、四區塊與 email 一致、表格可依總分／分項排序、
  顯示資料日期。email 照常寄送，網頁為新增而非取代。
...
除了 email，pipeline 每日另把四區塊輸出成 JSON 並發布到既有 GitHub Pages
（服務 `main` 根目錄）的**新子目錄 `/screener/`**，完全不動根目錄既有的
finance-navigator `index.html`。網址：

```
https://c20141223-art.github.io/finance-navigator-new/screener/
```

發布機制：daily-pipeline workflow 於寄完 email 後，把 `web/index.html` 與
當日 `web/data/*.json` 複製進 `main:/screener/`、由現存的每日封存檔重建
`data/index.json` 日期索引（歷史累積、不被單次執行清掉），只 commit／push
`/screener/`。因此 workflow 需要 `contents: write`，且若 `main` 有分支保護，
需允許 `github-actions[bot]` 推送（否則發布步驟會明確失敗，但 email 仍照
常寄出）。頁面為純前端、無後端、無外部服務相依，表格可依總分／分項排序、
顯示資料日期。
